...
def page(ids, next_url=None):
    return {"data": [{"id": str(i), "type": "entities"} for i in ids], "links": {"next": next_url}}
...
def test_cursor_spelling_and_filters_are_preserved(key):
    ...
    assert [x["id"] for x in BaseResource(client)._paginate("/entities", params=params, page_limit=1)] == ["1", "2"]
...
def test_metadata_cursor_and_full_page_retention():
    first = {"data": [{"id": "1"}], "included": [{"id": "related"}], "meta": {"page": {"next_cursor": "abc"}}}
...
def test_next_null_takes_precedence_over_stale_metadata():
    first = {"data": [], "links": {"next": None}, "meta": {"page": {"next_cursor": "abc"}}}
    client = Client([first])
...
def test_offset_uses_returned_page_number_not_increment():
    client = Client([page([1], "/v1/generated_reports?page[number]=7&page[size]=2"), page([2])])
    assert len(list(BaseResource(client)._paginate_offset("/generated_reports", page_size=2))) == 2
...
def test_query_only_links_and_multi_valued_filters():
    client = Client([page([1], {"href": "?page[cursor]=c&filter[x]=a&filter[x]=b"}), page([2])])
...
def test_external_pagination_link_is_rejected():
    class Session:
        def request(self, *args, **kwargs):
            return Client([page([1], "https://external.example/steal")])._request("GET", "/entities")
...
def test_poll_checks_immediately(clock):
    ...
    assert resource._poll_until_complete("id", lambda _: "Completed", lambda s: s == "Completed") == "Completed"
    assert clock.sleeps == []
...
def test_polling_sleeps_are_capped_at_deadline(clock):
    ...
    with pytest.raises(AddePyTimeoutError) as caught:
        resource._poll_until_complete("id", lambda _: "Queued", lambda _: False, initial_wait=3, max_wait=10, timeout=5)
...
def test_slow_status_response_cannot_succeed_after_deadline(clock):
    ...
    with pytest.raises(AddePyTimeoutError):
        BaseResource(Client([]))._poll_until_complete("id", check, lambda _: True, timeout=5)


@pytest.mark.parametrize("kwargs", [{"timeout": 0}, {"initial_wait": -1}, {"max_wait": 0}, {"backoff_factor": 0.5}, {"timeout": float("nan")}])
def test_invalid_polling_configuration_fails_before_request(kwargs):
    with pytest.raises(ValueError):
        BaseResource(Client([]))._poll_until_complete("id", lambda _: pytest.fail("no requests"), lambda _: False, **kwargs)
