...
def test_oauth_reuses_until_expiry_and_persists_rotated_refresh_token(monkeypatch):
    ...
    session.post.side_effect = [response({"access_token": "one", "refresh_token": "rotated", "expires_in": 100}), response({"access_token": "two", "expires_in": 100})]
    save = Mock()
    provider = OAuthTokenProvider(token_url="https://api.addepar.com/public/oauth2/token", client_id="id", client_secret="secret", refresh_token="initial", session=session, on_token_update=save)
...
@pytest.mark.parametrize("body,status", [({}, 200), ({"access_token": "x", "expires_in": 0}, 200), ({"client_secret": "sensitive"}, 401), ({}, 302)])
def test_invalid_refresh_response_is_explicit_and_redacted(body, status):
    session = Mock(spec=requests.Session)
    session.post.return_value = response(body, status)
    provider = OAuthTokenProvider(token_url="https://api.addepar.com/public/oauth2/token", client_id="id", client_secret="secret", refresh_token="refresh", session=session)
...
def test_token_provider_is_used_by_client():
    ...
    client = AddePy("example", "1", token_provider=provider, session=session, load_env=False)
    client.request("GET", "/entities")
    provider.assert_called_once_with()
    assert session.request.call_args.kwargs["headers"]["Authorization"] == "Bearer current"


@pytest.mark.parametrize("make_data", [lambda p: "a,b\n1,2\n", lambda p: b"a,b\n1,2\n", lambda p: io.StringIO("a,b\n1,2\n"), lambda p: io.BytesIO(b"a,b\n1,2\n"), lambda p: p])
def test_csv_inputs_work_without_pandas(make_data, tmp_path):
    ...
    assert ImportToolResource(client).create_import(make_data(path), "ATTRIBUTES") == "job"
...
def test_oauth_refresh_inherits_client_deadline(monkeypatch):
    ...
    refresh_session.post.return_value = response({"access_token": "current", "expires_in": 240})
    provider = OAuthTokenProvider(token_url="https://api.addepar.com/public/oauth2/token", client_id="id", client_secret="secret", refresh_token="refresh", session=refresh_session)
    api_session = Mock(spec=requests.Session)
    api_session.request.return_value = response({"data": []})
    client = AddePy("example", "1", token_provider=provider, session=api_session, load_env=False)
