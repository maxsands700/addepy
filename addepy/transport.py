...
from .exceptions import (
    AddePyError, AuthenticationError, ConflictError, ForbiddenError, GoneError,
    NotFoundError, ProtocolError, RateLimitError, RequestTimeoutError,
...
def retry_after(response: requests.Response) -> Optional[float]:
    ...
    value = response.headers.get("X-RateLimit-Retry-After") or response.headers.get("Retry-After")
    if value is None:
        return None
    try:
        seconds = float(value)
...
def raise_api_error(response: requests.Response) -> None:
    ...
    error_class = {
        ...
        404: NotFoundError, 409: ConflictError, 410: GoneError, 422: ValidationError,
...
class Transport:
    def __init__(self, *, base_url: str, headers: dict[str, str],
                 timeout: Any, max_retries: int, max_retry_wait: float,
        ...
        if (parsed.scheme != "https" or not parsed.netloc or parsed.username
            ...
            raise ValueError("base_url must be an HTTPS API URL without credentials, query, or fragment")
        ...
        self._deadline: ContextVar[Optional[float]] = ContextVar("addepy_deadline", default=None)

    @contextmanager
    def request_deadline(self, deadline: float) -> Iterator[None]:
        current = self._deadline.get()
        token = self._deadline.set(min(current, deadline) if current is not None else deadline)
        try:
            yield
        finally:
    ...
    def _validate_url(self, url: str, *, same_origin: bool) -> None:
        ...
        if (parsed.scheme != "https" or not parsed.hostname or parsed.username
                or parsed.password or parsed.fragment or (same_origin and origin != self._origin)):
            raise ValueError("API URLs must use HTTPS and the configured host; redirects cannot include credentials")

    def _timeout(self, configured: Any) -> Any:
        values = configured if isinstance(configured, tuple) else (configured, configured)
        if len(values) != 2 or any(not isinstance(v, (int, float)) or v <= 0 or not math.isfinite(v) for v in values):
            raise ValueError("timeout must be positive seconds or a (connect, read) pair")
        ...
        return TimeoutSauce(total=remaining, connect=min(values[0], remaining), read=min(values[1], remaining))
    ...
    def _send(self, method: str, url: str, **kwargs: Any) -> requests.Response:
        ...
        for _ in range(11):
            ...
            if external:
                ...
                prepared = requests.Request(method, url, headers={
                    "Accept": "*/*", "User-Agent": self.headers.get("User-Agent", "addepy"),
                }).prepare()
                response = session.send(
                    prepared, timeout=self._timeout(timeout), allow_redirects=False,
                    stream=kwargs.get("stream", False), verify=kwargs.get("verify", True),
                    cert=None, proxies={},
                )
            else:
                response = session.request(method, url, timeout=self._timeout(timeout), allow_redirects=False, **current)
            ...
            new_external = (parsed.scheme, parsed.hostname, parsed.port or 443) != self._origin
            ...
            if method not in ("GET", "HEAD") and response.status_code != 303:
                response.close()
                raise ProtocolError("Refusing to replay a write request across a redirect", response)
    ...
    def request(self, method: str, endpoint: str, *, retry: Optional[bool] = None,
        ...
        replayable = (not hasattr(kwargs.get("data"), "read")
                      and not isinstance(kwargs.get("data"), IteratorABC) and "files" not in kwargs)
        can_retry = (method in ("GET", "HEAD", "OPTIONS") if retry is None else retry) and replayable
        request_headers = dict(self.headers)
        request_headers.update(headers or {})
        for attempt in range(self.max_retries + 1):
            if self._token_provider is not None:
                if isinstance(self._token_provider, OAuthTokenProvider):
                    token = self._token_provider.get_token(deadline=self._deadline.get())
                else:
                    token = self._token_provider()
                if not isinstance(token, str) or not token:
                    raise AuthenticationError("Token provider returned an empty or invalid token")
            ...
            try:
                ...
                if not can_retry or attempt >= self.max_retries:
                    error = RequestTimeoutError if isinstance(exc, requests.Timeout) else TransportError
                    raise error(f"{method} request failed ({type(exc).__name__})") from exc
            else:
                if response.status_code < 400:
                    return response
                if not can_retry or attempt >= self.max_retries or response.status_code not in (429, 500, 502, 503, 504):
                    raise_api_error(response)
            delay = retry_after(response) if response is not None else None
            if delay is None:
                delay = (2 ** attempt) + random.uniform(0, 0.25)
            remaining = None if self._deadline.get() is None else self._deadline.get() - time.monotonic()
            if delay > self.max_retry_wait or (remaining is not None and delay >= remaining):
