...
class BaseResource:
    ...
    def _patch(
        self, endpoint: str, json: Any = None, **kwargs: Any
    ) -> requests.Response:
        ...
        *,
        ...
        if (
            any(
                ...
                for v in (initial_wait, max_wait, timeout)
            )
            or not math.isfinite(backoff_factor)
            or backoff_factor < 1
        ):
            raise ValueError(
                "Polling intervals/timeout must be positive; backoff_factor must be >= 1"
            )
    ...
    def iter_pages(
        ...
        *,
        ...
        cursor_parameter: str = "page[cursor]",
        ...
        pages = 0
        while True:
            fingerprint = current_endpoint + json.dumps(
                request_params, sort_keys=True, default=str
            )
            ...
            if not isinstance(document, dict) or "data" not in document:
                raise ProtocolError(
                    "Expected a JSON:API page containing data", response
                )
            yield document
            pages += 1
    ...
    def _items(
        self, pages: Any, max_items: Optional[int]
    ) -> Generator[Dict[str, Any], None, None]:
