...
class OAuthTokenProvider:
    ...
    def __init__(self, *, token_url: str, client_id: str, client_secret: str,
                 refresh_token: str, access_token: Optional[str] = None,
                 expires_in: float = 0, timeout: float = 30,
                 session: Optional[requests.Session] = None,
                 on_token_update: Optional[Callable[[dict[str, Any]], None]] = None) -> None:
        parsed = urlparse(token_url)
        if parsed.scheme != "https" or not parsed.netloc or parsed.username or parsed.password or parsed.fragment:
            raise ValueError("token_url must be an HTTPS URL without embedded credentials")
    ...
    def get_token(self, *, deadline: Optional[float] = None) -> str:
        ...
        try:
            ...
            if deadline is not None:
                ...
                if remaining <= 0:
                    raise RequestTimeoutError("OAuth refresh deadline exceeded")
                timeout = TimeoutSauce(total=remaining, connect=min(self._timeout, remaining), read=min(self._timeout, remaining))
            try:
            ...
            try:
                ...
                try:
                    ...
                    if not isinstance(token, str) or not token or lifetime <= 0 or not math.isfinite(lifetime):
                        raise ValueError("Invalid token response")
                except (ValueError, TypeError, KeyError) as exc:
                    raise AuthenticationError("OAuth response needs access_token and positive expires_in", response) from exc
